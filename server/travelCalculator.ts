...
function loadStateData(): Map<string, StateData> {
  ...
  try {
    ...
    for (let i = 3; i < data.length; i++) {
      ...
      const stateName = stateData.estado.toUpperCase();
      // Store by both full name and 2-letter code
      stateDataCache.set(stateName, stateData);
      const stateCode = stateCodeMap[stateName];
      if (stateCode) {
        stateDataCache.set(stateCode, stateData);
      }
    }
